...
describe("Lock Contract",function(){
    ...
    before(async function(){
        ...
        Token = await ethers.getContractFactory("YtuToken");
    ...
    it(" Deploys the contracts",async function() {
        ...
        expect(lock.address).to.be.properAddress;//same with to not be undefined
    });
    ...
    it(" Approves",async function(){
        ...
        expect(allowances[1]).to.be.equal(allowances[0]);
    });


    it("Reverts exceeding balance transfer",async function() {
        await expect(token.connect(user1).transfer(user2.address, ethers.utils.parseEther("300"))).to.be.reverted;
    })
